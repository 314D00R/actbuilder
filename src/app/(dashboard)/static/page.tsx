			...
			<div className='bg-orange-500/10 border border-orange-500/20 text-orange-500 p-4 rounded-xl flex gap-3 items-start shadow-sm'>
				...
				<div>
					<h3 className='font-bold text-sm mb-1'>
						Сталі дані (Не видаляються)
					</h3>
					\\
					<p className='text-xs opacity-90 leading-relaxed'>
						Ці дані залишаються збереженими навіть при створенні нового
						об&apos;єкта. Заповніть їх один раз для своєї робочої групи чи
						комісії.
					</p>
				</div>
			</div>
			...
			<Card className='border-l-4 border-l-orange-500 bg-card/60 backdrop-blur-sm shadow-md'>
				...
				<CardContent className='space-y-4'>
					...
					<div className='space-y-2 pt-2'>
						<Label className='text-xs uppercase text-muted-foreground'>
							Члени комісії (Родовий, через кому)
						</Label>
						<Input
							type='text'
					...
					{store.signs.map(sign => (
						...
						>
							<div className='space-y-2 w-full md:w-1/3'>
								...
								<Select
									value={sign.role}
									onValueChange={val => store.updateSign(sign.id, 'role', val)}
								>
									...
									>
										<SelectItem value='Голова комісії'>
											Голова комісії
										</SelectItem>
										<SelectItem value='Заступник голови комісії'>
											Заступник голови комісії
										</SelectItem>
										<SelectItem value='Секретар комісії'>
											Секретар комісії
										</SelectItem>
							...
							<div className='space-y-2 w-full md:flex-1'>
								...
								<Input
									...
									value={sign.name}
									onChange={e =>
										store.updateSign(sign.id, 'name', e.target.value)
									}
									className='w-full'
								/>
							</div>
