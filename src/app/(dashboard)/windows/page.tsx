...
export default function WindowsPage() {
  const { windows, buildings, updateWindow, calculateTotal, volumes } = useActStore();
  ...
  const handleAddWindow = () => {
    useActStore.setState((state) => {
      const nextId = state.windows.length > 0 ? Math.max(...state.windows.map((w) => w.id)) + 1 : 1;
      return {
        windows: [
          ...state.windows,
          {
      ...
      <Card className="border-l-4 border-l-primary bg-card/60 backdrop-blur-sm shadow-md">
        <CardHeader className="pb-4">
          <CardTitle className="text-sm font-bold flex items-center gap-2">
            <Square size={18} className="text-primary" /> Вікна та Двері
          </CardTitle>
          <p className="text-xs text-muted-foreground">
            Додавайте всі вікна і двері. Вписуйте ширину/висоту або відразу ПЛОЩУ вручну.
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          {windows.map((win) => (
            <div key={win.id} className="p-5 bg-secondary/30 border border-border rounded-xl space-y-5">
              <div className="flex items-center justify-between border-b border-border/60 pb-3">
                <div className="flex items-center gap-3 w-full max-w-xl">
                  <span className="text-xs font-bold px-2 py-1 bg-primary/10 text-primary rounded shrink-0">
                    Блок №{win.id}
                  </span>
                  <Select value={win.bld || ""} onValueChange={(val) => updateWindow(win.id, "bld", val)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    type="number"
                    value={win.qty || ""}
                    onChange={(e) => updateWindow(win.id, "qty", parseFloat(e.target.value))}
                    className="bg-background"
                  />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <Label className="text-xs uppercase text-muted-foreground">Тип</Label>
                  <Select value={win.type} onValueChange={(val) => updateWindow(win.id, "type", val)}>
                    <SelectTrigger className="bg-background">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent position="popper" side="bottom">
                      <SelectItem value="frame">Вікно: Рама+Укоси (п.9,10)</SelectItem>
                      <SelectItem value="glass">Вікно: Тільки скло (п.8)</SelectItem>
                      <SelectItem value="door_mp">Двері м/п (п.9 без укосів)</SelectItem>
                      <SelectItem value="door_in_6">Двері міжкімнатні (п.6)</SelectItem>
                      <SelectItem value="door_out_7">Двері вхідні метал. (п.7)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                {win.type === "frame" && (
                  <div className="space-y-2">
                    <Label className="text-xs uppercase text-muted-foreground">Тип укосів</Label>
                    <Select value={win.sType} onValueChange={(val) => updateWindow(win.id, "sType", val)}>
                      <SelectTrigger className="bg-background">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent position="popper" side="bottom">
              ...
              <div className="flex items-center gap-3 flex-wrap bg-background p-3 rounded-lg border border-border">
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    type="number"
                    value={win.w || ""}
                    onChange={(e) => updateWindow(win.id, "w", parseFloat(e.target.value))}
                    className="w-20 h-8"
                  />
                </div>
                <span className="text-muted-foreground/50">×</span>
                <div className="flex items-center gap-2">
                  <Label className="text-xs uppercase text-muted-foreground">В:</Label>
                  <Input
                    type="number"
                    value={win.h || ""}
                    onChange={(e) => updateWindow(win.id, "h", parseFloat(e.target.value))}
                    className="w-20 h-8"
                  />
                </div>
                <span className="text-muted-foreground/50">=</span>
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    type="number"
                    value={win.res_w || ""}
                    onChange={(e) => updateWindow(win.id, "res_w", parseFloat(e.target.value))}
                ...
                {win.type === "frame" && (
                  <>
                    ...
                    <div className="flex items-center gap-2">
                      <Label className="text-xs uppercase text-muted-foreground">Глиб:</Label>
                      <Input
                        type="number"
                        value={win.d || ""}
                        onChange={(e) => updateWindow(win.id, "d", parseFloat(e.target.value))}
                        className="w-16 h-8"
                      />
                    </div>
                    <span className="text-muted-foreground/50">→</span>
                    <div className="flex items-center gap-2">
                      <Label className="text-xs font-bold uppercase text-primary">Укоси:</Label>
                      <Input
                        type="number"
                        value={win.res_s || ""}
                        onChange={(e) => updateWindow(win.id, "res_s", parseFloat(e.target.value))}
                        className="w-24 h-8 font-bold text-primary border-primary/50 bg-primary/5"
                      />
                    </div>
                  </>
          ...
          >
            <Plus size={18} className="mr-2" /> Додати вікно / двері
          </Button>
        </CardContent>
      </Card>
    </div>
